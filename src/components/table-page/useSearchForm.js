...
export function useInitSearchForm(props) {
    ...
    let handleSubmit = () => { // 提交
        ...
        props.service.showList(temp).then((res) => {
            console.log(res);
            if (res.code === 0) {
    ...
    let caculateTableHeight = (tableHeight, tableRef) => {
        nextTick().then(() => {
            let total = tableRef.value.$el.offsetTop + tableRef.value.$el.offsetParent.offsetTop + tableRef.value.$el.nextElementSibling.offsetHeight + 10;
